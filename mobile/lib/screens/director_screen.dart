...
class _DirectorScreenState extends State<DirectorScreen>
  ...
  Future<void> _loadBranchesAndSummary() async {
    try {
      ...
      for (int i = 0; i < branches.length; i++) {
        ...
        final bName = (b['name'] ?? b['branchName'] ?? 'Chi nhánh ${i + 1}')
            .toString();
        ...
        if ((bSummary == null || bSummary['data'] == null) &&
            bId.isNotEmpty &&
            bId != bCode) {
        ...
        if (bRevData != null) {
          final net =
              double.tryParse(
                (bRevData['netRevenue'] ?? bRevData['totalRevenue'] ?? 0)
                    .toString(),
              ) ??
              0;
      ...
      final netRev =
          double.tryParse((revData?['netRevenue'] ?? 0).toString()) ?? 0;
      final finalChainRev = netRev > 0 ? netRev : totalChainRevenue;

      String formattedChainRev = '';
      if (finalChainRev >= 1000000000) {
        formattedChainRev =
            '${(finalChainRev / 1000000000).toStringAsFixed(3)} tỷ ₫';
      } else if (finalChainRev >= 1000000) {
        formattedChainRev =
            '${(finalChainRev / 1000000).toStringAsFixed(1)} triệu ₫';
      } else {
      ...
      if (mounted) {
        setState(() {
          ...
          for (var po in poList) {
            ...
            final itemsSummary = itemsList
                .map(
                  (i) =>
                      "${i['medicineName'] ?? i['name']} (x${i['qty'] ?? i['quantity'] ?? 1})",
                )
                .join(", ");
            _poPendingApprovals.add({
              'id': po['_id'] != null
                  ? 'PO-${po['_id'].toString().substring(po['_id'].toString().length - 5).toUpperCase()}'
                  : po['id'] ?? 'PO-001',
              'rawId': po['_id'] ?? po['id'],
              'branch': po['branch'] ?? 'Cơ sở Tổng',
              'supplier': po['supplier'] ?? 'Nhà cung cấp Dược',
              'amount': '${(po['totalAmount'] ?? 50000000).toString()} ₫',
              'date': po['createdAt'] != null
                  ? po['createdAt'].toString().substring(0, 10)
                  : '12/06/2026',
              'items': itemsSummary.isNotEmpty
                  ? itemsSummary
                  : 'Các loại thuốc kháng sinh và thiết bị y tế',
            });
          }
        });
      }
  ...
  Future<void> _approvePO(int index, String poId) async {
    ...
    await ApiService.approvePurchaseOrder(
      rawId,
      "APPROVE",
      paymentType: "PAID",
    );
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Đã PHÊ DUYỆT thành công đơn nhập hàng $poId',
  ...
  Future<void> _rejectPO(int index, String poId) async {
    ...
    await ApiService.approvePurchaseOrder(
      rawId,
      "REJECT",
      rejectionReason: "Từ chối bởi Giám Đốc trên di động",
    );
    if (mounted) {
      setState(() {
        _poPendingApprovals.removeAt(index);
                ...
                children: [
                  ...
                  const Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          'Dự Báo Nhu Cầu AI Gemini (Real Backend)',
                          style: TextStyle(
                            fontWeight: FontWeight.bold,
                            fontSize: 17,
                          ),
                        ...
                        Text(
                          'Phân tích chuỗi cung ứng & xu hướng thời tiết mạn',
              ...
              ...(predictions.isNotEmpty
                  ? predictions.map((item) {
                      ...
                      final advice =
                          item['recommendation'] ??
                          item['advice'] ??
                          'Dự báo tiêu thụ tăng do giao mùa, đề xuất chuẩn bị thêm tồn kho.';
                  ...
                  : [
                      _buildPredictionItem(
                        'Amoxicillin 500mg',
                        '+28% nhu cầu',
                        'Khuyên dùng: Nhập thêm cho các cơ sở chi nhánh',
                      ),
                      _buildPredictionItem(
                        'Panadol Extra',
                        '+15% nhu cầu',
                        'Dự báo tiêu thụ mạnh vào dịp giao mùa sắp tới',
                      ),
                    ]),
              const SizedBox(height: 16),
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  onPressed: () {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text(
                          'Đã gửi đề xuất tự động tạo PO nhập hàng cho Thủ kho!',
                        ),
      ...
      appBar: AppBar(
        title: const Column(
          ...
          children: [
            ...
            Text(
              'hãy bắn 1 request kiểm thử  & HIỆU SUẤT CHUỖI',
              style: TextStyle(
                fontSize: 10,
                fontWeight: FontWeight.bold,
        ...
        children: [
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      boxShadow: [
                        BoxShadow(
                          color: _primaryGreen.withValues(alpha: 0.24),
                          blurRadius: 14,
                      ...
                      child: OutlinedButton.icon(
                        onPressed: () {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text(
                                'Đã xuất Báo cáo Doanh thu & Tồn kho PDF thành công!',
                              ),
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Text(
                        '${_poPendingApprovals.length} chờ duyệt',
                        style: TextStyle(
                ...
                _isLoading
                    ...
                    ? Container(
                        ...
                        child: const Text(
                          'Hiện không có đơn PO nào chờ duyệt',
                          style: TextStyle(
                            color: Colors.grey,
                            fontWeight: FontWeight.bold,
                          ),
                                ...
                                child: Padding(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Text(
                                        'Nhà cung cấp: ${po['supplier'] ?? 'Dược phẩm OPC'}',
                                        style: TextStyle(
                                          color: Colors.grey.shade700,
                                          fontSize: 12,
                                        ),
          ...
          SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                const Text(
                  'Hiệu Suất Chi Nhánh Trực Thuộc',
                  style: TextStyle(
                    fontSize: 15,
                    fontWeight: FontWeight.bold,
                    color: _ink,
                        ...
                        child: Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 12),
                              Text(
                                '${_lowStockItems.length} sản phẩm',
                                maxLines: 1,
                                overflow: TextOverflow.ellipsis,
                                style: const TextStyle(
                                  fontSize: 18,
            ...
            children: [
              ...
              Expanded(
                child: _stockTransfers.isEmpty
                    ? const Center(
                        child: Text(
                          'Chưa có lịch sử luân chuyển kho liên chi nhánh.',
